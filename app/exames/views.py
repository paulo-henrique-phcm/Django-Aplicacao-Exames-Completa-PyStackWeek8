...
from django.contrib.auth.decorators import (
    login_required,
)  # é um decorador que só roda a função se o usuario estiver logado
...
def solicitar_exames(request):
    tipos_exames = (
        TiposExames.objects.all()
    )  # objects acessa os dados da tabela araves dos models
    if request.method == "GET":
        return render(request, "solicitar_exames.html", {"tipos_exames": tipos_exames})
    elif request.method == "POST":
        exames_id = request.POST.getlist("exames")
        solicitar_exames = TiposExames.objects.filter(id__in=exames_id)
        print(solicitar_exames)
        preco_total = 0
        ...
        return render(
            ...
            "solicitar_exames.html",
            {
                ...
                "solicitacao_exames": solicitar_exames,
...
def acesso_medico(request, token):
    ...
    pedidos = PedidosExames.objects.all()
